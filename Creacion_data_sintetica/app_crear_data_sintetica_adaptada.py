...
def load_pixabay_data():
    try:
        pixabay_df = pd.read_csv('/Users/n.arcos89/Desktop/Bootcamp_Data/DIVA_DIGITAL_Proyecto Final/Data/publicaciones_pixabay.csv')
        # Extraer ID_Publicacion, Imagen Y Fecha
        return pixabay_df[['id_post', 'Imagen', 'Fecha']].values.tolist()
...
for i in range(0, numbers):
    # Seleccionar aleatoriamente un registro completo del CSV de Pixabay
    if pixabay_data:
        ...
        fecha_real = random_post[2]  # Fecha del CSV
        
        # Convertir la fecha del CSV al formato correcto si es necesario
        try:
            # Intentar parsear la fecha (ajusta el formato según tu CSV)
            if isinstance(fecha_real, str):
                fecha_objeto = pd.to_datetime(fecha_real).date()
            else:
                fecha_objeto = fecha_real
        except:
            # Si hay problema con la fecha, usar una aleatoria como fallback
            fecha_objeto = fake.date_between(start_date=fecha_inicio, end_date=fecha_fin)
    else:
    ...
    data.append({
        'id_post': post_id,
        'Fecha': fecha_objeto.strftime('%Y-%m-%d'),  # Cambiar fecha_aleatoria por fecha_objeto
        'Hora': fake.time(),
